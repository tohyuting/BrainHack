...
document.addEventListener('init', function(event) {
  ...
  if (page.id === 'home') {
    page.querySelector('#english').onclick = function() {
      document.querySelector('#myNav').pushPage('EnglishLanding.html', {data: {title: 'English Home Page'}});
    };
  ...
  } else if (page.id === 'ChineseHome') {
    page.querySelector('#ChineseHomeButton').onclick = function() {
      document.getElementById("ChineseHomeAudio").play();
    }
